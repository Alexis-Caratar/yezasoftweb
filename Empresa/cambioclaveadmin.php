...
if (isset($_GET['claveactual'])) $claveactual=$_GET['claveactual'];else $claveactual='';
if (isset($_GET['clavenueva'])) $clavenueva=$_GET['clavenueva'];else $clavenueva='';
if (isset($_GET['confirmarclave'])) $confirmarclave=$_GET['confirmarclave'];else $confirmarclave='';
?>
<div class="container col-md-5"><br>
    <label>USUARIO ACTUAL: <?=$_SESSION['rol']?></label>
    ...
    <form name="formulario" action="PrincipalAdmin.php?CONTENIDOADMIN=Empresa/actualizarClave.php&usuario=<?=$usuario?>"method="post">
        ...
        <tr><th>Usuario</th><th><input class="form-control" type="text" name="usuarionuevo" placeholder="Nuevo usuario" value="<?=$_SESSION['rol']?>" required autofocus > </th></tr>
        ...
        <input class="form-control" type="hidden" name="usuario" value="<?=$usuario?>">
    </form>
</div>